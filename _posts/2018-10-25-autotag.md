...
Now suppose that, after drawing a post at random, we draw a single word from the post at random (without peeking at exactly which post we've drawn).  The probability that we've drawn any given post is now $$p(doc|word)$$.  The *information gain* of the word is the difference between $$H$$ up above and the entropy of this new distribution $$p(doc|word)$$:  

$$\Delta H = H[p(doc)] - H[p(doc|word)].$$ 
